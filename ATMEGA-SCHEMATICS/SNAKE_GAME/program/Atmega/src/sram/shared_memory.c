...
void loadScore(){
    ...
    if(sram_map.score.player_name != NULL){sramWriteStringPoll(&spi, (char *)sram_map.score.player_name,    SCORE_PNAME(base), 12);}
    if(sram_map.score.game_name != NULL){sramWriteStringPoll(&spi, (char *)sram_map.score.game_name,      SCORE_GNAME(base), 12);}
